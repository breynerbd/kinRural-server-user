...
export const requestLoan = async (req, res) => {
  try {
    ...
    const sumaCuotas = prestamosActivos.reduce(
      (acc, l) => acc + parseFloat(l.cuota_mensual || 0),
      0,
    );

    if (sumaCuotas + cuota > user.ingresos_mensuales * 0.4)
...
export const getMyLoans = async (req, res) => {
  try {
    ...
    const response = loans.map((loan) => {
      const cuotasPagadas = loan.loan_installments.filter(
        (i) => i.estado === "PAGADA",
      ).length;

      const cuotasPendientes = loan.loan_installments.filter(
        (i) => i.estado !== "PAGADA",
      ).length;
